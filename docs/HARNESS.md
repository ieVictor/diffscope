...
`get_impact_graph` takes no filter; its parameters shape the walk. `file` names one changed file to root the graph at, `function_id` names one function to root it at instead — the two are mutually exclusive — and with neither every changed file is a root; `direction` is one of `upstream`, `downstream`, `both` (default `both`); `relations` is any subset of `imports`, `tested_by`, `calls`, `contains`, and `re_exports` (default: every supported relation); `depth` defaults to `1` and is clamped to 1–3; `view` is one of `delta`, `base`, `target` (default `delta`); `max_nodes` and `max_edges` default to `30` and `60` and are clamped to 3–100 and 3–200; and `render` is any subset of `diff` and `mermaid` (default: none), returned as `data.dependency_diff` and `data.mermaid`. Inputs that would ask a question DiffScope does not answer are rejected rather than approximated: the schema advertises those nine parameters and no others, and a root the analysis does not contain is answered as `invalid_params` — an unknown `file` with the closest known changed paths, an unknown `function_id` with the closest known function ids. A relation outside the four is rejected with a message naming them, a `render` name other than `diff` or `mermaid` is rejected, and naming both `file` and `function_id` is rejected with a message saying the two are mutually exclusive. Answering any of them with an empty graph would report "nothing found" where the truth is "never asked".
