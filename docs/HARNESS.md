...
| `get_impact_graph` | `file`, `function_id`, `direction`, `relations`, `depth`, `view`, `max_nodes`, `max_edges`, `render` | The relationships a comparison added, removed, or left in place: module imports and test links walked from one changed file or the changed set, and — rooted at a function — the calls it makes in its own file and the ones it receives there. Optional dependency-diff and Mermaid renderings. |
...
`get_impact_graph` takes no filter; its parameters shape the walk. `file` names one changed file to root the graph at, `function_id` names one function to root it at instead — the two are mutually exclusive — and with neither every changed file is a root; `direction` is one of `upstream`, `downstream`, `both` (default `both`); `relations` is any subset of the supported `imports`, `tested_by`, `calls`, `contains`, `re_exports`, and `possible_call`. The default excludes `possible_call`; name it explicitly to request the `property_name_match` heuristic at confidence `0.5`, which is never a `calls` edge. `depth` defaults to `1` and is clamped to 1–3; `view` is one of `delta`, `base`, `target` (default `delta`); `max_nodes` and `max_edges` default to `30` and `60` and are clamped to 3–100 and 3–200; and `render` is any subset of `diff` and `mermaid` (default: none), returned as `data.dependency_diff` and `data.mermaid`.
...
| `get_impact_graph` | The relationships the comparison added, removed, or left in place — module imports and test links, and, rooted at a `function_id`, the calls it makes in its own file and receives there — with optional dependency-diff and Mermaid renderings. |
...
- `get_impact_graph` returns `{"root":...,"graph":{...},"completeness":{...},"visualization":{...}}`, with `root` `null` when the request named none and the graph is centered on the changed set. A named root carries `kind`, `id`, and `path`: a file root is `kind: "module"` at `module:<path>`, a function root is `kind: "function"` at `function:<function_id>` with `path` naming the file that declares the function, so a function graph says which function and which module it is about. The graph carries `nodes`, `edges`, `truncated`, `omitted`, and `reasons`; a group node additionally carries `group: {"size":<count>,"role":<role>}`. `completeness` is always present with `scan_truncated_files`, `unresolved_specifiers`, `unresolved_calls`, and `relations_supported`; counts cover the examined scope before grouping and budgets, and supported relations are independent of the request. `visualization` carries `recommended` and `reasons`. `dependency_diff` and `mermaid` are added when `render` asks for them.
...
- `invalid_params`: a parameter is missing, is not one of its accepted values, or is a cursor that does not match this request's analysis, method, schema version, or query. The message names the field and what it accepts. A root the analysis does not contain is one of these: an unknown `function_id` is answered with the closest known function ids, the way an unknown identity is answered by `get_function_change`.
